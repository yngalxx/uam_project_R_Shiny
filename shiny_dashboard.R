...
usa_china_pop <- merge(x = usa_pop, y= china_pop, by = 'Year',
                       all = TRUE,
...
ui <- dashboardPage(
  ...
  dashboardBody(
    tabItems(
      tabItem('population',
        fluidRow(
          box(plotOutput('plot_pop_china', height = 350)),
    
          box(plotOutput('plot_pop_usa', height = 350))
        ),
        fluidRow(
          box(
  ...
  output$plot_migrants <- renderPlot({
    ggplot() +
      ...
      geom_line(data = usa_china_pop, aes(x = Year, 
                                          ...
                                          colour = 'China')) +
      scale_color_manual(name = 'Country', values = c(
        'USA' = 'blue',
        'China' = 'red')) +
